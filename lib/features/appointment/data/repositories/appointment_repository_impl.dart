...
class AppointmentRepositoryImpl implements AppointmentRepository {
  ...
  @override
  Future<Either<Failure, List<BusyRangeEntity>>> getAvailability(int doctorId, String date) async {
    try {
      final availability = await remoteDataSource.getAvailability(doctorId, date);
  ...
  Future<Either<Failure, AppointmentEntity>> createAppointment(AppointmentEntity appointment) async {
    try {
      ...
      final createdAppointment = await remoteDataSource.createAppointment(model);
  ...
  @override
  Future<Either<Failure, Map<String, dynamic>>> getMedicalRecords({int page = 1, int limit = 10, String? search}) async {
    try {
      final result = await remoteDataSource.getMedicalRecords(page, limit, search);
      
      // Transform data list to Entities
      final List<dynamic> dataList = result['data'] ?? [];
      final appointments = dataList.map((json) => AppointmentModel.fromJson(json)).toList();
  ...
  @override
  Future<Either<Failure, void>> simulatePayment(int appointmentId) async {
    try {
