...
class _QueueMonitorScreenState extends State<QueueMonitorScreen> {
      ...
      body: BlocBuilder<FrontDeskBloc, FrontDeskState>(
        builder: (context, state) {
          if (state is FrontDeskLoading) {
            ...
            final allQueue = state.queue;

            // Separate queues by type
            final doctorQueue = allQueue
                .where((e) => e.queueType == 'Doctor')
                .toList();
            final polyclinicQueue = allQueue
                .where((e) => e.queueType == 'Polyclinic')
                .toList();

            // Get statistics from combined queues (doctor + polyclinic only)
            ...
            final inConsultationCount = combinedQueue
                ...
                .length;
            final completedCount = combinedQueue
      ...
      child: Column(
        ...
        children: [
          ...
          if (queue.isEmpty)
            ...
            ...queue.map(
              (entry) => _buildQueueItem(entry, hasInConsultation, accentColor),
            ),
        ],
      ),
    );
  ...
  Widget _buildQueueItem(
    ...
    // Disable call button if someone else is in consultation
    final canCall = entry.status == 'Waiting' && !hasInConsultation;
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    entry.practitioner ?? entry.polyclinic ?? '-',
  ...
  void _showQueueDetailDialog(QueueEntryModel entry) {
    ...
    // Format the calledAt time
    String formattedTime = '-';
    if (entry.calledAt != null && entry.calledAt!.isNotEmpty) {
      try {
        final dateTime = DateTime.parse(entry.calledAt!);
        formattedTime = DateFormat('dd MMM yyyy, HH:mm').format(dateTime);
      } catch (_) {
        formattedTime = entry.calledAt!;
      }
    }
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (entry.practitioner != null)
                _buildDetailRow('Doctor', entry.practitioner!),
              if (entry.polyclinic != null)
                _buildDetailRow('Polyclinic', entry.polyclinic!),
              _buildDetailRow('Queue Time', formattedTime),
              if (entry.isPriority == 1)
                _buildDetailRow('Priority', 'Yes', isHighlighted: true),
  ...
  ({String label, Color color}) _getStatusInfo(String status) {
    switch (status) {
      case 'Waiting':
        return (label: 'Waiting', color: Colors.amber);
      case 'Called':
