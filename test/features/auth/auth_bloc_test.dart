...
import 'package:app/features/auth/domain/usecases/google_login_usecase.dart';
...
void main() {
  ...
  group('AuthBloc', () {
    ...
    blocTest<AuthBloc, AuthState>(
      'emits [AuthGuest] when guest login requested',
      build: () => authBloc,
      act: (bloc) => bloc.add(GuestLoginRequested()),
      expect: () => [isA<AuthGuest>()],
    );
  });
}
