...
class AppointmentBloc extends Bloc<AppointmentEvent, AppointmentState> {
  ...
  Future<void> _onGetAvailability(
    GetAvailabilityRequested event,
    Emitter<AppointmentState> emit,
  ) async {
  ...
  Future<void> _onSimulatePayment(
    SimulatePaymentRequested event,
    Emitter<AppointmentState> emit,
  ) async {
    final result = await simulatePayment.execute(event.appointmentId);
    result.fold((failure) => emit(AppointmentError(failure.message)), (_) {
      // Trigger refresh
  ...
  Future<void> _onCancelAppointment(
    ...
    final result = await cancelAppointment.execute(event.appointmentId);
    result.fold((failure) => emit(AppointmentError(failure.message)), (_) {
      // Trigger refresh
      add(GetAppointmentsRequested());
    });
